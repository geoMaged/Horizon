...
function Article(){
    ...
    const [isDelete,setDelete] = useState(false);
     ...
     useEffect(()=>{
        ...
        .then(response =>{
             ...
             setIsMyArticle((localStorage.getItem('email') === authorEmail) ? true:false);
    ...
    function handleClick(e){
        e.preventDefault();
        if(bookmark){
            axios.delete('/api/articles/bookmark',{
                headers:{
                    'authorization':`Bearer ${localStorage.getItem('token')}`
                },
                data:{
            ...
            axios.post('/api/articles/bookmark',{article:id},{
                headers:{
                    'authorization':`Bearer ${localStorage.getItem('token')}`
                }
            })
    ...
    return(
        <div className='article' >
            ...
            {isMyArticle ? <Link to={{ pathname:'/editarticle', state:{article,id} }} > <button className='bookmark-btn btn btn-dark'><i className="fas fa-edit"> </i> Edit article</button> </Link> : null }
            {
            isMyArticle ?
                <button className='bookmark-btn btn btn-dark' onClick={handleDelete}><i className="fas fa-trash-alt"></i>  Delete article</button>: null 
             }
